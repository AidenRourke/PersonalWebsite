...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async getTopTrack() {
                const res = await axios.get(`https://arcane-beyond-48143.herokuapp.com/top_track?time_range=${this.timerange.value}`);
                if (res.data) {
                    this.song = res.data;
                    this.current = false;
                }
        ...
        async created() {
            try {
                ...
                this.evtSource = new EventSource(`https://arcane-beyond-48143.herokuapp.com/stream`);
